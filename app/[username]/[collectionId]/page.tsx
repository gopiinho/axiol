...
export default async function CollectionPage({
  ...
  params: Promise<{ username: string; collectionId: Id<"collections"> }>;
}) {
  const { username, collectionId } = await params;
  const convex = getServerConvexClient();

  const [collection, items] = await Promise.all([
    convex.query(api.collections.getById, { id: collectionId }),
    convex.query(api.items.listByCollection, { collectionId }),
  ]);
  ...
  return (
    <main className="home-font-primary min-h-screen flex justify-center p-4">
      ...
      <div className="w-full max-w-4xl">
        <Link href={`/${username}`} className="inline-block mb-8">
